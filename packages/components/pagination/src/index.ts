...
const StPagination = defineComponent({
    ...
    render() {
        return h('div' , {class : 'st-pagination'} , [
            this.single && h('ul' , {},[
                h('li' , {class : ['st-pagination--left' ,this.$props.value === 1 && 'st-pagination--disabled'], onClick : this.handleClick(this.$props.value - 1)} , [
                    h('i' , {class : 'st-icon--left' }),
                    h('span' ,  { disabled : this.$props.value === 1 } , this.$props.prevText)
                ]),
                this.simple ? h('li' , {class : 'st-pagination--item'} , this.value + '/' + this.pageSizeW) :
                    this.pages.map(item => {
                        return  h('li' , {class : 'st-pagination--item', onClick : this.handleClick(item)}, [
                            h('span' , {} , item)
                        ])
                    }),
                h('li' , {class : ['st-pagination--right',this.$props.value === this.pageSizeW && 'st-pagination--disabled'], onClick : this.handleClick(this.$props.value - 1)} , [
